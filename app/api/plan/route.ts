...
export async function POST(req: Request) {
  ...
  const colCat = findCol(headers, ["category", "vehicle category", "cat", "الفئة", "تصنيف", "نوع"]);
  ...
  for (const g of groups) {
    ...
    planRows.push({
      ...
      Start: dateFrom,
      End: dateTo,
      "Device Mix": Object.entries(g.devices)
        .map(([k, v]) => `${k}:${v}`)
        .join(" | "),
    });
  ...
  if (opt.includeRecommendations !== false) {
    ...
    sheet.columns = Object.keys(outRows[0])
      ...
      .map((k) => ({
        header: k,
        key: k,
        width: Math.min(42, Math.max(14, k.length + 2)),
      }));
  ...
  if (opt.includePdf !== false) {
    ...
    const title =
      lang === "ar"
        ? "ALRAKEEN | خطة تركيب وتوصية أجهزة Teltonika CAN"
        : "ALRAKEEN | Teltonika CAN Recommendation + Installation Plan";
    ...
    if (includePlan) {
      y -= 6;
      page.drawText(lang === "ar" ? "خطة التركيب (Top 10):" : "Installation plan (Top 10):", {
        x: 40,
        y,
        size: 11,
        font: bold,
      ...
      page.drawText(
        lang === "ar" ? `إجمالي الأيام: ${totalDays}` : `Total days: ${totalDays}`,
        { x: 40, y: 60, size: 10, font: bold }
      );
    }

    if (includeCost) {
      page.drawText(lang === "ar" ? "تكاليف (تقديرية):" : "Costs (estimate):", {
        x: 320,
        y: 260,
        size: 11,
        font: bold,
